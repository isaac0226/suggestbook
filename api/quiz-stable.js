...
async function researchBook(apiKey, model, book) {
  const prompt = `Google 검색으로 다음 책을 조사하세요. 같은 제목의 다른 책을 섞지 마세요. 입력한 글쓴이·그림 작가·옮긴이·출판사와 일치하는 자료를 우선하세요. 초등 독서 문제에 사용할 이야기 내용만 수집하세요. 등장인물, 장소, 행동, 사건의 순서, 원인과 결과, 감정, 핵심 메시지만 포함하고 수상 경력, 판매 기록, 출판 정보, 작가 이력, 그림 제작 기법은 facts에 넣지 마세요. JSON만 출력하세요.\n제목: ${book.title}\n글쓴이·그림·옮긴이: ${book.people || '알 수 없음'}\n출판사: ${book.publisher || '알 수 없음'}\nISBN: ${book.isbn || '알 수 없음'}\n형식: {"summary":"초등학생이 이해할 수 있는 이야기 요약","facts":["이야기 속 사실"],"confidence":0.0}`;
  try {
